...
# set.seed(666)
# 
# trainIndex <- createDataPartition(df_dummified_2017_pca_vars$unem, p = 0.8, list = FALSE)
...
```{r split the data again}
set.seed(666)
trainIndex <- 1:nrow(df_dummified_2017_pca_vars)
```
...
```{r train rf using parallel programming}
rf_model <- train(unem ~., 
                  data = df_dummified_2017_pca_vars, 
                  method = "rf", 
                  trControl = ctrl, 
                  weights = ifelse(
                      df_dummified_2017_pca_vars$unem == 1, 5, 1), 
                  nthread = detectCores())
```

```{r train GBM  using parallel programming}
gbm_model <- train(unem ~., 
                   data = df_dummified_2017_pca_vars, 
                   method = "gbm", 
                   trControl = ctrl, 
                   weights = ifelse(
                       df_dummified_2017_pca_top_vars$unem == 1, 5, 1), 
                   verbose = FALSE, 
                   nthread = detectCores())
```

```{r stop parallelizing}

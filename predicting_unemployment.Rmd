...
df2015 <- read.csv("cps_2015.csv", sep = ";") %>%
  dplyr::select(
    age,
    female,
    wbhaom,
...
Next we combine the three datasets to one single dataframe 'df'
```{r}
df <- bind_rows(df2017,df2016, df2015) %>% 
  filter(!is.na(unem))
```

Then, we impute NAs with zeros '0' and convert to factor. The variables that have NA are "hours", "fulltimelw" and "hprsmort".
...
Here we ...... Plot proportion of dependent variable 'unem'
...
In order to increase the models ability to predict positive cases we create a more balanced dataset. We increase the proportion of positive cases for 'unem' from originally 3% to 13% be dropping 200000 rows of negative cases. 
...
Here we used using 2015, 2016 and 2017 datasets .................
...
# Define the function to train and predict the KNN model
train_and_predict <- function(train, test) {
  # Fit model and train
  model_knn <- train(unem ~ ., 
                     data = train, 
                     method = "knn")
  
  # Predict on test data
  model_knn <- predict(model_knn, 
                       newdata = test, 
                       type = "prob")[,2]
...
# Split the data into chunks and perform the prediction in parallel
chunk_size <- floor(nrow(df) / num_cores)
chunks <- split(df, rep(1:num_cores, 
                        each = chunk_size, 
                        length.out = nrow(df)))

model_knn_parallel <- foreach(chunk = chunks, 
                              .combine = c, 
                              .packages = "caret") %dopar% {
  train_and_predict(train = chunk, 
                    test = test_set)
}
...
model_knn <- unlist(model_knn_parallel)
...
# Create confusion matrix and calculate performance measures
cm_knn <- confusionMatrix(predicted_class_knn, 
                          test_set$unem)
cm_knn
```
...
```{r}
# ROC curve
pred.knn <- prediction(model_knn, 
                       test_set$unem)
perf.knn <- performance(pred.knn,
                        measure = "tpr", 
                        x.measure = "fpr") 

...
The variables that have NA are "hours", "fulltimelw" and "hprsmort":

```{r}
summary(is.na(df2018))
```
Then, we impute NAs with zeros '0' and convert to factor. 
```{r NA imputation }
...
```{#r}
# ROC curve
pred.knn.2018 <- prediction(model.knn, 
                       df2018$unem)


perf.knn.2018 <- performance(pred.knn.2018,
                        measure = "tpr", 
                        x.measure = "fpr") 
